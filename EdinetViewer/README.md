...
＊＊過去の書類一覧の取得はわざと時間がかかるように設計しています。再度右クリックメニューを表示すると中断の表示に変更されるようになっており、キャンセルが可能です。
...
4. MySQLなどとは違いSqlite3は同時書き込みに弱くシングルスレッドを想定したデータベースとされているようです。 
- バックグラウンドで過去データ一覧取得してデータベースWriteを行なっている最中にフォアグラウンドで同じデータベースにアクセスする可能性があり、場合によっては競合でエラーが発生したりデータベースがクラッシュすることもありえます。
- バックグラウンド作動中は書き込みのある操作（当日分の書類一覧読み込み、書類のダウンロード）はなるべく避ける方が望ましいと考えています。
- トラブルがある場合にはバックグラウンドを廃止することも考えています。
